...
void setup() //Inicjalizacja okna i modelu
{
  ...
  World[Side/2][Side/2].Set(STARTG,STARTG,STARTG); //Inicjalizowanie pierwszym ziarnem 
...
{
  //Zapis tego co jest
  output.println(Step+"\t"+RGB_Counter); //Zapisz statystyki do pliku 
    
  //Nowy stan
  if(!Stop)
  {
    videoExport.saveFrame(); //Kolejna ramka video
    int M=Side*Side;
    for(int i=0;i<M;i++)
    {
      ...
      if(World[Y][X]!=null)
      {
         ...
         if(0<=Xt && Xt<Side && 0<=Yt && Yt<Side
            &&  World[Yt][Xt]==null)
          {
            ...
            {
               Stop=true; 
               videoExport.saveFrame(); //Ostatnia ramka video
               videoExport.endMovie(); //I "koniec filma"
            }
          }  
      }
  }

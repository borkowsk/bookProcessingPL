/// Wizualizacja świata..
/// @date 2026-06-12 (modyfikacja)
//*/////////////////////////////////////////////////////////////////////////////

void visualise(World currWorld) //< W tej procedurze symulacji jest jeszcze jeden trudny do znalezienia błąd. Duży plus dla znalazcy!
{
  float CSide=width/WSide;
  float HSide=CSide/2.0;
  
  for(int row=0;row<WSide;row++)
    for(int col=0;col<WSide;col++)
      if(currWorld.plane[row][col]!=null)
      {
        fill(currWorld.plane[row][col].getColor()); 
        noStroke();
        ellipse(col*CSide-HSide,row*CSide-HSide,CSide,CSide);
      }
      else
      {
        noFill(); stroke(0);
        rect(col*CSide,row*CSide,CSide,CSide);
      }
      
   // Kiedy zakończymy rysowanie agenta, grafika pozostaje z opcją `noStroke`, która nie jest domyślna.   
   stroke(0); // To jest lepsze rozwiązanie.    
}
